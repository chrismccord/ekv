defmodule EKV.AntiEntropyTest do
  ...
  describe "anti-entropy healing" do
    ...
    test "stale inflight markers expire so summary refresh resumes" do
      ...
      assert sent_node_id == assigned_node_id(peers, node_a)
    ...
    test "summary reply restores member identity when current shard only knows progress" do
      peers = TestCluster.start_peers(2)
      [{_, node_a}, {_, node_b}] = peers
      ekv_name = unique_name(:anti_entropy_summary_reply_node_id)
      on_exit(fn -> TestCluster.stop_peers(peers) end)
      on_exit(fn -> cleanup_data(peers, ekv_name) end)

      start_cluster(peers, ekv_name, anti_entropy_interval: @manual_anti_entropy_interval)

      assert assigned_node_id(peers, node_b) ==
               TestCluster.member_node_identity(node_a, ekv_name, node_b)

      assert :ok = TestCluster.clear_member_node_id(node_a, ekv_name, node_b)
      assert :ok = TestCluster.clear_member_node_identity(node_a, ekv_name, node_b)
      assert :ok = TestCluster.trigger_anti_entropy(node_a, ekv_name)

      TestCluster.assert_eventually(fn ->
        state = TestCluster.replica_state(node_a, ekv_name)

        Map.get(state.member_node_ids, node_b) == assigned_node_id(peers, node_b) and
          TestCluster.member_node_identity(node_a, ekv_name, node_b) ==
            assigned_node_id(peers, node_b)
      end)
    end
    ...
    test "third-origin relay uses node-id presence when current peer mapping is absent" do
      peers = TestCluster.start_peers(3)
      [{_, node_a}, {_, node_b}, {_, node_c}] = peers
      ekv_name = unique_name(:anti_entropy_member_presence_node_id)
      on_exit(fn -> TestCluster.stop_peers(peers) end)
      on_exit(fn -> cleanup_data(peers, ekv_name) end)

      start_cluster(peers, ekv_name, anti_entropy_interval: @manual_anti_entropy_interval)

      write_many(node_a, ekv_name, "presence_identity_gap", 3)

      TestCluster.assert_eventually(fn ->
        TestCluster.keys_count(node_c, ekv_name, "presence_identity_gap/") == 3
      end)

      assert :ok = TestCluster.force_local_progress(node_b, ekv_name, node_a, 0)
      assert :ok = TestCluster.clear_member_node_id(node_b, ekv_name, node_a)
      assert :ok = TestCluster.clear_member_node_identity(node_b, ekv_name, node_a)
      assert :ok = TestCluster.trace_shard_sends(node_b, ekv_name, self())
      assert :ok = TestCluster.trigger_anti_entropy(node_b, ekv_name)
  ...
  describe "anti-entropy HWM safety" do
    ...
    test "live replication batches to upgraded peers and falls back for peers without the feature" do
      ...
      assert :ok = TestCluster.rpc!(node_a, EKV, :put, [ekv_name, "live_batch/1", "v1"])
      assert :ok = TestCluster.rpc!(node_a, EKV, :put, [ekv_name, "live_batch/2", "v2"])

      batch_messages = collect_trace_messages([], 500)

      assert Enum.any?(batch_messages, fn
               {:replication_batch, ^node_a, 0, _origin, ["live_batch/1", "live_batch/2"],
                _destination} ->
                 true
      ...
      TestCluster.assert_eventually(fn ->
        TestCluster.rpc!(node_b, EKV, :get, [ekv_name, "live_batch/1"]) == "v1" and
          TestCluster.rpc!(node_b, EKV, :get, [ekv_name, "live_batch/2"]) == "v2"
      end)

      assert :ok = TestCluster.set_remote_features(node_a, ekv_name, node_b, [], 0)
      _ = collect_trace_messages([], 100)
  ...
  describe "anti-entropy suppression / gating" do
    ...
    test "anti-entropy clears stale node_id down markers for currently connected members before later reconnect churn" do
      peers = TestCluster.start_peers(2)
      [{_, node_a}, {_, node_b}] = peers
      ekv_name = unique_name(:anti_entropy_connected_marker_cleanup)
      tombstone_ttl = 700
      on_exit(fn -> TestCluster.stop_peers(peers) end)
      on_exit(fn -> cleanup_data(peers, ekv_name) end)
      ...
      TestCluster.assert_eventually(fn ->
        Map.has_key?(TestCluster.replica_state(node_a, ekv_name).remote_shards, node_b)
      end)
      ...
      assert TestCluster.member_down_marker(node_a, ekv_name, marker_key) == stale_down_since
    ...
    test "overdue node_id marker alone does not quarantine a current live member on reconnect" do
      peers = TestCluster.start_peers(2)
      [{_, node_a}, {_, node_b}] = peers
      ekv_name = unique_name(:anti_entropy_overdue_id_marker_reconnect)
      tombstone_ttl = 700
      on_exit(fn -> TestCluster.stop_peers(peers) end)
      on_exit(fn -> cleanup_data(peers, ekv_name) end)
      ...
      assert TestCluster.member_down_marker(node_a, ekv_name, id_marker_key) == stale_down_since
      assert TestCluster.member_down_marker(node_a, ekv_name, name_marker_key) == nil

      assert TestCluster.rpc!(node_a, EKV.MemberPresence, :member_nodes, [ekv_name])
             |> Enum.sort() ==
               Enum.sort([node_a, node_b])

      assert TestCluster.rpc!(node_a, EKV.MemberPresence, :member_origin_known?, [
               ekv_name,
               node_b_id
             ])
      ...
      state = TestCluster.replica_state(node_a, ekv_name)
      refute Map.has_key?(state.remote_shards, node_b)
      refute MapSet.member?(state.quarantined_members, node_b)
      ...
      TestCluster.assert_eventually(fn ->
        ...
        Map.has_key?(state.remote_shards, node_b) and
          not MapSet.member?(state.quarantined_members, node_b) and
          TestCluster.member_down_marker(node_a, ekv_name, id_marker_key) == nil and
          TestCluster.member_down_marker(node_a, ekv_name, name_marker_key) == nil
      end)
    end
