...
| `:anti_entropy_interval` | `30_000` (30 sec) | Member mode only. Periodic background repair for already-connected members. Re-runs the normal member handshake + delta/full sync path to heal missed replication without waiting for reconnect. `false`/`nil` disables it. |
