...
echo "==> Starting replica3..."
elixir --name replica3@127.0.0.1 --cookie "$COOKIE" --erl "$KERNEL_OPTS" \
  -S mix run --no-halt -e "Bench.Replica.start()" &
REPLICA3_PID=$!

cleanup() {
  echo "==> Stopping replicas..."
  kill "$REPLICA1_PID" "$REPLICA2_PID" "$REPLICA3_PID" 2>/dev/null || true
  wait "$REPLICA1_PID" "$REPLICA2_PID" "$REPLICA3_PID" 2>/dev/null || true
}
trap cleanup EXIT
...
elixir --name coordinator@127.0.0.1 --cookie "$COOKIE" --erl "$KERNEL_OPTS" \
  -S mix run -e "Bench.CAS.run(shards: $SHARDS)"
